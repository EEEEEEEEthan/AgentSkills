...
**原则**：新增 API 时，优先使用局部变量、局部常量和私有辅助方法，避免把一次性状态提升为类字段。

**默认做法**：
- 仅在“跨多次调用复用且可证明收益”时新增类字段（如场景缓存、对象池引用）
- 单次调用即可完成的数据，放在方法内部，不上移为成员
- 如果新增成员只是为一个 API 服务，先评估是否可改为局部变量或复用现有成员
